...
## 6단계 논문 프로토콜 부분 재현 결과

- 논문에 공개된 40/10/10×5 시간 분할과 5-fold 임계값 선택 절차를 고정 Random Forest로 10시드 실행했다.
- 무작위 test 평균은 slip rate 1.16%, volume reduction 71.58%로 평균 gate에 미달했고, 시드별로는 4/10회 통과했다.
- 미래 5구간의 평균 slip rate는 1.94~7.60%였으며, 50개 시드-구간 중 두 업무목표를 동시에 만족한 경우는 없었다.
- 무작위 분할에서 미래 평가로 갈 때 실패하는 방향은 재현했지만, 논문 RFC2의 무작위 test volume 89.7%와는 차이가 컸다.
- 인용 코드, Bayesian search space와 최종 hyperparameter가 공개 상태에서 확보되지 않아 결과를 완전 재현이 아닌 프로토콜 부분 재현으로 한정한다.
